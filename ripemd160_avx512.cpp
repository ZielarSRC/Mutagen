...
// Round macros for 16 parallel lanes
...
void Transform(__m512i* s, uint8_t* blk[16]) {
  ...
  // Load 16 message blocks
  for (int i = 0; i < 16; ++i) {
    w[i] = LOADW(i);
  }

  // Round 1-16 (Left Pipeline)
...
static const uint64_t sizedesc_64 = 64 << 3;
...
void ripemd160avx512_64(unsigned char* i0, unsigned char* i1, unsigned char* i2, unsigned char* i3,
  ...
  // Padding dla 64-bajtowych bloków
  for (int i = 0; i < 16; ++i) {
    memcpy(bs[i] + 64, pad, 56);
    memcpy(bs[i] + 120, &sizedesc_64, 8);
  }
