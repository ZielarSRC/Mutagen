...
extern "C" void shiftR(unsigned char n, uint64_t *d, uint64_t carryIn);

#ifndef WIN64
// Inline implementation of imm_umul for non-Windows systems
inline void imm_umul(const uint64_t *a, uint64_t b, uint64_t *r) {
  uint64_t carry = 0;
...
// Inline implementation of shiftR with carry
inline void shiftR(unsigned char n, uint64_t *d, uint64_t carryIn = 0) {
  if (n == 0) return;
...
void Int::DivStep62(const Int *u, const Int *v, int64_t *eta, int *pos, int64_t *uu, int64_t *uv,
...
#define SWAP(tmp, x, y) \
  ...
  _v.m128i_u64[1] = 1;
#else
  // POPRAWKA: Użyj union zamiast type-punning dla strict-aliasing compliance
  union {
    __m128i vec;
    int64_t arr[2];
  } u_union, v_union;
...
void Int::ModSquare(const Int *a) {
  Int p;
...
void Int::ModMul(const Int *a, const Int *b) {
  Int p;
...
void Int::SetupField(const Int *n, Int *R, Int *R2, Int *R3, Int *R4) {
...
// ------------------------------------------------
void Int::MontgomeryMult(const Int *a) {
...
void Int::MontgomeryMult(const Int *a, const Int *b) {
  // Compute a*b*R^-1 (mod n),  R=2^k (mod n), k = Msize*64
  // a and b must be lower than n
  // See SetupField()
