...
Point Secp256K1::NextKey(Point &key) { return AddDirect(key, G); }
...
void Secp256K1::GetHash160(int type, bool compressed, Point &pubKey, unsigned char *hash) {
  ...
  pubkeys[0] = &pubKey;
  hashes[0] = hash;
  GetHash160_Batch16(type, compressed, pubkeys, hashes);
}
...
Point Secp256K1::ParsePublicKeyHex(std::string str, bool &isCompressed) {
  ...
  if (str.size() == 66 && (str[0] == '0' && (str[1] == '2' || str[1] == '3'))) {
    ...
    uint8_t buff[32];
    for (int i = 0; i < 32; ++i) buff[i] = std::stoi(hex.substr(i * 2, 2), nullptr, 16);
    p.x.Set32Bytes(buff);
    ...
    std::string hex = str.substr(2);
    uint8_t buff[64];
    for (int i = 0; i < 64; ++i) buff[i] = std::stoi(hex.substr(i * 2, 2), nullptr, 16);
    p.x.Set32Bytes(buff);
